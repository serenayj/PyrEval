...
#Wasih 07-04-21: Make global variables
base_dir = os.path.dirname(os.path.realpath(__file__))
config = configparser.ConfigParser()
#config.read('parameters.ini')
config.read(os.path.join(base_dir, 'parameters.ini'))
#config.read('/home/wasih7/pyreval-flask-main/PyrEval/parameters.ini')
...
results_file = config.get('Paths', 'OutputFile')
ext_dir = config.get('Paths', 'ExtDir')
log_dir = config.get('Paths', 'LogDir')
pyramid_path = config.get('Paths', 'PyramidPath')
num_models = config.get('Params', 'NumModels')
...
#Wasih 04-05-21: Add function to support flask application

def getIndividualScoreFunc(peer_summary):
    print(peer_summary)
    #config = configparser.ConfigParser()
    #config.read('/home/wasih7/pyreval-flask-main/PyrEval/parameters.ini')
    # TODO: user-changeable
    ...
    #first check, whether peer_summary is a string
    if type(peer_summary) is str:
        #dump text into a file under Peer/
        #create directories used by scoring (sizes and temp if not present)
...
#Wasih 04-05-21: Add flag to just run under peer mode (not running model summaries)

def splitsent(params, peer_mode):
...
def preprocess(params, peer_mode):
